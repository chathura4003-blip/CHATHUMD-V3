...
async function generateAIResponse(prompt, imageBuffer = null, mimeType = "image/jpeg", systemInstruction = "", options = {}) {
  ...
  // These third-party endpoints receive the *raw user prompt*, including
  // any private context the user shared with the bot. They are off by
  // default and can be opted into with AI_PUBLIC_FALLBACK=true.
  if (!imageBuffer && config.AI_PUBLIC_FALLBACK) {
    const backupUrls = [
  ...
  // Surface a clearer signal when nothing was tried because every
  // provider key was missing. Helps operators figure out why the bot
  // suddenly went silent on AI replies.
  if (!hasGeminiKey() && !hasOpenRouterKey() && !hasGroqKey() && !config.AI_PUBLIC_FALLBACK) {
    return {
      text: null,
      model: null,
      missingKeys: true,
...
async function handleChat(sock, msg, from, q, sender) {
  ...
  if (!result?.text) {
    ...
    if (result?.missingKeys) {
      errorHelp += "📍 *හේතුව:* AI API keys configure කරලා නැහැ. \n";
      errorHelp += "💡 Owner: Set GEMINI_API_KEY / OPENROUTER_API_KEY / GROQ_API_KEY in environment, or enable AI_PUBLIC_FALLBACK=true (not recommended for private chats).";
    } else {
