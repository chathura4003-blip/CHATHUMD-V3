...
function load() {
  ...
  const primary = tryParseFile(DB_PATH);
  if (primary && !primary.__parseError) {
    cache = ensureSchema(primary);
    return cache;
  }

  if (primary?.__parseError) {
    // Primary db.json is corrupt — try the .bak written before the
    // last successful flush.
    const backup = tryParseFile(BACKUP_PATH);
    if (backup && !backup.__parseError) {
      cache = ensureSchema(backup);
      try {
        // Quarantine the corrupt file so an operator can inspect it.
        fs.renameSync(DB_PATH, `${DB_PATH}.corrupt-${Date.now()}`);
      } catch {}
      console.error(`[db] db.json was corrupt (${primary.__parseError.message}); recovered from ${BACKUP_PATH}.`);
      return cache;
    }
    console.error(`[db] db.json is corrupt (${primary.__parseError.message}) and no usable backup exists. Starting with an empty schema.`);
  }
...
function flush() {
  ...
  // Snapshot the previous good copy before overwriting so a crash
  // mid-write or a corrupt JSON serializer can be recovered from
  // db.json.bak on next load.
  try {
