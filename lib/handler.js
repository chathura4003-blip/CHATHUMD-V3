...
async function handleCommand(sock, msg, from, text, disabledModules = [], context = {}) {
  ...
  try {
        ...
        if (aiCmd && typeof aiCmd.generateAIResponse === "function") {
          (async () => {
            try {
              ...
              // Rate Limit Check (Max 5 msgs/min per user, Max 15/min per chat)
              // — Exempt Owner & Premium. Per-chat cap stops one busy
              // group from monopolising the AI provider quota.
              ...
              if (!isOwnerUser && !isPrem) {
                ...
                const chatLimit = (aiRateLimits.get(chatKey) || []).filter(t => now - t < 60000);
                if (chatLimit.length >= 15) {
                  return logger(`[AI-Limit] Chat rate limit hit for ${from.split('@')[0]}`);
                }
                chatLimit.push(now);
                aiRateLimits.set(chatKey, chatLimit);
              }
    ...
    // Each command module can declare any of: ownerOnly, groupOnly,
    // pmOnly, adminOnly, premiumOnly. Anything not declared is allowed
    // by default so existing commands keep working unchanged.
    if (cmd.ownerOnly && !isOwnerUser) {
      await msgMgr.sendTemp(sock, from, "🔒 This command is owner-only.", 4000);
      return true;
    }
    if (cmd.groupOnly && !isGroup) {
      await msgMgr.sendTemp(sock, from, "👥 This command can only be used inside a group.", 4000);
      return true;
    }
    if (cmd.pmOnly && isGroup) {
      await msgMgr.sendTemp(sock, from, "💬 This command can only be used in private chat.", 4000);
      return true;
    }
    if (cmd.premiumOnly) {
      const isPrem = db.getUser(sender)?.premium === true;
      if (!isOwnerUser && !isPrem) {
        await msgMgr.sendTemp(sock, from, "⭐ This command is for premium users only.", 4000);
        return true;
      }
    }
    if (cmd.adminOnly && isGroup) {
      try {
        ...
        if (!isOwnerUser && !isAdmin) {
          await msgMgr.sendTemp(sock, from, "🛡️ Only group admins can use this command.", 4000);
          return true;
        }
      } catch {
        // If we can't read group metadata, fall through and let the
        // command itself decide rather than blocking on a transient
        // Baileys error.
      }
    }
