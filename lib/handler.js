...
async function handleCommand(sock, msg, from, text, disabledModules = [], context = {}) {
  ...
  try {
    ...
    if (!cmdText.startsWith(prefix)) {
      ...
      // ── Pro: Mention Reply ─────────────────────────────────────────────
      // Fires when bot is @-mentioned and a custom mention-reply text is configured.
      // Sends one reply per sender per 60s, then short-circuits the AI auto-reply
      // path below so the user does not receive two responses to the same mention.
      let mentionReplySent = false;
      const mentionReplyText = context.mentionReply || appState.getMentionReply();
      if (mentionReplyText && botNumber) {
        const mentionList = msg.message?.extendedTextMessage?.contextInfo?.mentionedJid || [];
        const wasMentioned = mentionList.some((j) => (j || '').startsWith(botNumber))
          || cmdText.includes(`@${botNumber}`);
        if (wasMentioned && !msg.key.fromMe) {
          const lastMentionAt = aiRateLimits.get(sender + ":mention") || 0;
          if (Date.now() - lastMentionAt > 60000) {
            aiRateLimits.set(sender + ":mention", Date.now());
            try {
              await sock.sendMessage(from, {
                text: mentionReplyText,
                mentions: [sender],
              }, { quoted: msg });
              mentionReplySent = true;
            } catch (e) {
              logger(`[MentionReply] Send failed: ${e.message}`);
            }
          } else {
            // Cooldown active — still treat the mention as handled so AI doesn't
            // double-respond a second later.
            mentionReplySent = true;
          }
        }
      }
      if (mentionReplySent) return true;
        ...
        if (aiCmd && typeof aiCmd.generateAIResponse === "function") {
          (async () => {
            try {
              ...
              const customInstr = (context.aiSystemInstruction !== undefined && context.aiSystemInstruction !== null && context.aiSystemInstruction !== '')
                ? String(context.aiSystemInstruction)
                : appState.getAiSystemInstruction();
              const maxWordsRaw = (context.aiMaxWords !== undefined && context.aiMaxWords !== null)
                ? parseInt(context.aiMaxWords)
                : appState.getAiMaxWords();
              const maxWords = Number.isFinite(maxWordsRaw) && maxWordsRaw > 0 ? maxWordsRaw : 30;
